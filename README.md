...
By default, if any vulnerability at `medium` or higher is seen, the build fails. If Grype configuration sets `fail-on-severity`, the action uses that value instead. The `severity-cutoff` input overrides both and can be set to `negligible`, `low`, `medium`, `high`, or `critical`:
...
| `fail-build`        | Fail the build if a vulnerability is found at or above the effective severity cutoff.                                                                                                                                                                           | `true`        |
...
| `severity-cutoff`   | Optionally specify the minimum vulnerability severity to trigger a failure. Overrides Grype's configured `fail-on-severity`; when neither is set, defaults to `medium`. Valid choices are `negligible`, `low`, `medium`, `high`, and `critical`.                     | config / medium |
